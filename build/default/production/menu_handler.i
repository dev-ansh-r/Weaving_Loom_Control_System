...
# 25 "./menu_handler.h"
int option = 0;
char arr[4][16]={"WELCOME","1.LOOM SETTINGS","2.DHOTI SETTINGS","3.RESET"};
int loom_w=0, loom_s=0, loom_p=0;
...
void Dothi_settings(){
    ...
    if(dhoti_steps <= 60)
    {
        for(int i=1;i<=dhoti_steps;){
            LcdDisplayNumber(1,0,i);
            LcdDisplayString(1,1,".");
            LcdDisplayNumber(2,0,i+1);
            LcdDisplayString(2,1,".");
            for(int j=3;j>=0;--j)
                dhoti_values[i] = increment_one(dhoti_values[i], j, 1,3);

            for(int j=3;j>=0;--j)
                dhoti_values[++i] = increment_one(dhoti_values[++i], j, 2, 3);
        }
    }
}
