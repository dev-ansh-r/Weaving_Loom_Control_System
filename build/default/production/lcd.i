...
unsigned char LcdDisplaytwo(unsigned char RowNum, unsigned char Pos, unsigned short Data);
...
DhotiSetting_t __DhotiSetting_t[99];
...
static char DataArray2[3];
static char DataArray1;

static void SendLcdCommand( unsigned char Command)
{
    LATD &= ~(1<<2);
    LATD = (Command & 0XF0)|(LATD &0x0F) ;

    LATD |= (1<<3);
    _delay((unsigned long)((5)*(20000000/4000.0)));
    LATD &= ~(1<<3);

    LATD = (unsigned char)((Command & 0X0F) << 4) |(LATD &0x0F);
...
{
    LATD |= (1<<2);
    LATD = (Data & 0XF0) |(LATD &0x0F) ;

    LATD |= (1<<3);
    _delay((unsigned long)((5)*(20000000/4000.0)));
    LATD &= ~(1<<3);

    LATD = (unsigned char)((Data & 0X0F) << 4) |(LATD &0x0F);
...
unsigned char LcdDisplaytwo(unsigned char RowNum, unsigned char Pos, unsigned short Data)
{
    DataArray2[0] = (unsigned char)(((Data)/10)+48);
    DataArray2[1] = (unsigned char) ((((Data))%10)+48);
    DataArray2[2] = 0;
    if( ValidateNumber(DataArray2))
    {
        LcdDisplayString(RowNum,Pos,DataArray2);
        return 1;
    }
    else
    {
       LcdDisplayString(RowNum,Pos,"Hello");
       return 0;
    }
}
